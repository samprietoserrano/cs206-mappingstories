...
def determine_relevance(locations_in_groups, package_option, components, weights, simple=False):
    ...
    for loc in list(set(all_locations)):
        score_list = list(np.array([0.0] * len(components)))
        for component in components:
            title_ner = locations_in_groups[0]
            unweighted_score = determine_component(component, loc, all_locations, title_ner, n_entities, package_option, cluster_info)

            for weight in weights:
                this_weight = weight[components.index(component)]
                score_list[weights.index(weight)] += this_weight * unweighted_score
        scores_dict[loc] = score_list

    return [Counter(dict(zip(scores_dict.keys(), values))).most_common(3) for values in zip(*scores_dict.values())]
