...
def determine_relevance(locations, sentences, title, package_option, components, weights_list, excel_function = None, simple=False, backend_catch=False):
    """
    ...
    for loc in list(set(locations)):
        ...
        # scores_dict[loc] = [a*b for a, b in zip(weights_list[0], score_parts)]

    return location_json, [Counter(dict(zip(scores_dict.keys(), values))).most_common(3) for values in zip(*scores_dict.values())]
    # return {}, []
    # return location_json, []
    # excel_function(scores_dict)
    # return scores_dict


def analyze_transcript(text_info, package_option, components, weights_list, excel_function = None, only_ner=False, backend_catch=False):
    """
    ...
    if backend_catch:
        return determine_relevance(locations, sentences, text_info[0], package_option, components, weights_list, excel_function = excel_function, backend_catch=True)
