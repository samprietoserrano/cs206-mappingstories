...
from geodata_packages import determine_coords

def frequency(loc, all_locations, n_counts):
    """
    Determine the proportional frequency of a location in the text.
    """    
    n_entities, n_length = n_counts
    n_occurrences = all_locations.count(loc)
    return n_occurrences/n_entities
    # return (n_occurrences/n_entities)*0.7 + ((n_occurrences*len(loc))/n_length)*0.3


def title_mention(loc, title):
    """
    Determine the inclusion of a location in the title segment.

    NOTE: This function could also use fuzzy matching for more flexibility.
    """ 
    return loc in title
...
def determine_component(component,loc, all_locations, title, n_counts, package_option, cluster_info, graph_info):
    """
    Determine the value of a component for a location.
    """
    if component == "freq":
        return frequency(loc, all_locations, n_counts)
    elif component == "segment":
        return title_mention(loc, title)
